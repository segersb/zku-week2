...
describe("MerkleTree", function () {
    ...
    it("Verify a proof for another Merkle tree", async function () {
        const Input = {
            "leaf": "1",
            "path_elements": ["2", "14763215145315200506921711489642608356394854266165572616578112107564877678998", "14693904821945502268578313651525098196765636411922213115469821563817117273617"],
            "path_index": ["0", "0", "0"]
        }
        const { proof, publicSignals } = await groth16.fullProve(Input, "circuits/circuit_js/circuit.wasm","circuits/circuit_final.zkey");
        ...
        expect(await merkleTree.verify(a, b, c, input)).to.be.false;
    });
});
